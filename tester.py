...
def test_place_and_search(nodes, files):
	print ("\n\t --* Testing File Placement and File Search *--\n")
		...
		for j in range(len(nodes)):
			...
			if ((fileHashes[i] <= nodes[j].key and fileHashes[i] > nodes[j-1].key) or
				...
				if files[i] not in nodes[j].files:
					correct = False
	if correct:
		print ("All files placed successfully. \t(+7)")
		points += 7
	else:
		print ("Some or all files placed incorrectly. \t(0)")
		return 0

	print ("Testing File Search")
	remove_files(files)
	time.sleep(4)
	print ("Checking for a file placed on network DHT")

	if nodes[0].get(files[0]) == None:
		print ("Could not retrieve a file placed on network DHT")
		print ("File Search failed. \t(0)")
		return points
...
nodes, p1 = initiate(p)
nodes, p2 = test_join(nodes, p)
generate_files(files)
p3 = test_place_and_search(nodes, files)
nodes, p4 = test_file_rehashing(nodes, files, start_port+5)
nodes, p5 = test_leave(nodes, files)
nodes, p6 = test_failure_tolerance(nodes, files)

print ("\nTotal points: ", p1+p2+p3+p4+p5+p6, "/ 40", file=sys.stderr)
